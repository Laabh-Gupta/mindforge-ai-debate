...
import { consumeAccountLimit, getDatabase, withOwnerTransaction } from "./database.server";
import { getVisitorId } from "./request-security.server";
import { PracticeSchema } from "@shared/practice-schema";
import { canComplete, type PracticeSession } from "@shared/practice-types";
...
export async function handleAccounts(request: Request): Promise<Response | null> {
  ...
  if (path.startsWith("/api/auth/")) {
    if (mutation) {
      ...
      const globalLimit = await consumeAccountLimit("auth:global", 300, 60000);
      if (!globalLimit.allowed)
        return Response.json(
          { message: "Too many attempts. Please wait and try again." },
          { status: 429, headers: { "Retry-After": String(globalLimit.retryAfter) } },
        );
      const limits = [globalLimit, await consumeAccountLimit("auth:visitor:" + visitor, 30, 60000)];
  ...
  if (path === "/api/community" && request.method === "GET" && !url.searchParams.has("mine")) {
    ...
    const entries = await db
      .prepare(
        `
      SELECT c.display_name, COUNT(*)::int AS sessions, (COUNT(*)*100)::int AS xp
      ...
      GROUP BY c.user_id, c.display_name ORDER BY xp DESC, c.display_name ASC LIMIT 100
    `,
      )
      .all(since);
    return json(entries);
  ...
  if (path === "/api/community") {
    if (request.method === "GET")
      return json(
        (await db.prepare("SELECT listed FROM community_profiles WHERE user_id=?").get(owner)) || {
          listed: 0,
        },
      );
  ...
  const reset = async () =>
    Number(
      (
        (await db.prepare("SELECT cleared_at FROM practice_resets WHERE user_id=?").get(owner)) as
  ...
  if (request.method === "GET") {
    ...
    return json({ sessions: rows.map((row) => JSON.parse(row.payload)), clearedAt: await reset() });
  }
  if (request.method === "DELETE") {
    return withOwnerTransaction(owner, async () => {
      const clearedAt = Date.now();
      await db.prepare("DELETE FROM practice_sessions WHERE user_id=?").run(owner);
      await db
        .prepare(
  ...
  return withOwnerTransaction(owner, async () => {
    if (practice.startedAt > (await reset())) {
      await db
        .prepare(
